...
\item [--] Fatigue effect, since participants have to examine more code snippets. This was exacerbated in our case, due to the larger number of atom candidates. Although we could have mitigated this problem by limiting the number of atom candidates shown to the subjects, our decision was to ask the participants to evaluate pairs of code snippets for every atom candidate---and thus collecting a large number of answers.
\end{itemize}}
\revised{
...
\begin{itemize}
\item[+] Although there is some data dependency (since we collect different observations from a given subject),	analyzing the data in the Latin square	design allows us to isolate confounding variables better.
\item[+] There is no possibility of learning/practice effect and the fatigue effect is reduced since each participant is only exposed to one version of each code snippet. 
\end{itemize}}
\revised{
\noindent
The main disadvantages of a between subjects study are the following:
\begin{itemize}
  \item[-] Statistical significance tends to be lower. 
  \item[-] Does not account for variance within groups.
\end{itemize}
The use of a Latin square mitigates some of the disadvantages intrinsic to within subjects design. For example, since less work is required from participants, it was possible to obtain more data for the latin square study than we could get for the repeated measures study. This increases its statistical power, making it possible to detect smaller effect sizes.}
