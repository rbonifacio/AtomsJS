...
\captionof{lstlisting}{Code snippet from \textsc{NervJS/taro} project. Line 2 in the left hand side of the figure contains two
  ...
  as Control Flow} and \emph{Post-increment}. In the right hand side, we present the alternative code without atoms of confusion. We could simplify the code on the left even more, by replacing the \texttt{reduce} recursive pattern by a functional composition between the \texttt{filter} and
\texttt{length} patterns (e.g., \texttt{res = properties.filter(p => p).length}).}
\end{figure*}
