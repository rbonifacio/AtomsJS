...
The goal of this paper is twofold. First, we aim to investigate whether atoms of confusion that were identified in statically typed languages also cause confusion to developers of a dynamically typed language. We leverage JavaScript as a representative example of dynamic programming languages. 
...
JavaScript's programming culture differs from that of languages such as \clang and Java, particularly due to its dynamic capabilities and weaker type system. At the same time, the language is not so different that previously identified atoms cannot be represented in it.\castor{The previous sentence is really bad.} 
